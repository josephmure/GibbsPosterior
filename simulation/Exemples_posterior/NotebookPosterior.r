...
evaluePriorBerger <- function( x_connus, regularite, longueur_correlation, indice_derive, tendance)
    {
...
##    inverseMatriceCorrelation <- solve(matriceCorrelation)
	...
	ProjectionBerger <- diag(rep(1,nrow(matriceCorrelation))) - tendance %*% solve(t(tendance) %*% solve(matriceCorrelation,tendance),t(tendance) %*% solve(matriceCorrelation))

    MatriceAuxiliaire <- try(solve(matriceCorrelation, ProjectionBerger %*% matriceDerivee))
...
evaluePosteriorBerger <- function( x_connus, y_connus, regularite, longueur_correlation, indice_derive, tendance)
{
    Noyau <- creeMaternTensorise ( variance= 1, longueur= longueur_correlation, regularite= regularite)
	...
	Prior <- evaluePriorBerger( x_connus, regularite, longueur_correlation, indice_derive, tendance)
	
	Posterior <- Prior * det(matriceCorrelation)^(-1/2) * det(t(tendance) %*% solve(matriceCorrelation,tendance))^(-1/2) * (t(ProjectionBerger %*% y_connus) %*% solve(matriceCorrelation, ProjectionBerger %*% y_connus) )^(-(nrow(x_connus) - ncol(tendance))/2)

	Posterior
}


evaluePriorIsotropeREL1 <- function( x_connus, regularite, longueur_correlation, tendance)
    {
...
##    inverseMatriceCorrelation <- solve(matriceCorrelation)
    ...
    matriceDerivee <- creeMatriceCovariance(x1= x_connus, x2= x_connus, noyau= creeMaternIsotropeDeriveLongueur( variance=1, longueur= Noyau$parametres$longueur, regularite= Noyau$parametres$regularite, indice_derive=1) )

injectionOrthogonal <- qr.Q(qr(tendance),complete=TRUE)[,(1+ncol(tendance)):ncol(matriceCorrelation)]

matriceCorrelation <- t(injectionOrthogonal) %*% matriceCorrelation %*% injectionOrthogonal
matriceDerivee <- t(injectionOrthogonal) %*% matriceDerivee %*% injectionOrthogonal
    ...
    {
        Prior_carre <- sum( diag(MatriceAuxiliaire %*% MatriceAuxiliaire) ) - 1/(nrow(x_connus)-ncol(tendance)) * (sum(diag(MatriceAuxiliaire)))^2
        Prior <- sqrt(Prior_carre)
    }
    Prior
}
...
evaluePriorREL1 <- function( x_connus, regularite, longueur_correlation, indice_derive, tendance=NULL)
    {
    Noyau <- creeMaternTensorise ( variance= 1, longueur= longueur_correlation, regularite= regularite)
...
nombre_fonctions_base <- 0
if(!is.null(tendance)) {

nombre_fonctions_base <- ncol(tendance)
...
matriceDerivee <- t(injectionOrthogonal) %*% matriceDerivee %*% injectionOrthogonal
    ...
    {
        Prior_carre <- sum( diag(MatriceAuxiliaire %*% MatriceAuxiliaire) ) - 1/(nrow(x_connus)-nombre_fonctions_base) * (sum(diag(MatriceAuxiliaire)))^2
        Prior <- sqrt(Prior_carre)
    }
...
evaluePosteriorREL1 <- function( x_connus, y_connus, regularite, longueur_correlation, indice_derive, tendance)
{
    Noyau <- creeMaternTensorise ( variance= 1, longueur= longueur_correlation, regularite= regularite)
...
injectionOrthogonal <- qr.Q(qr(tendance),complete=TRUE)[,(1+ncol(tendance)):ncol(matriceCorrelation)]
injectionParallele <- qr.Q(qr(tendance),complete=TRUE)[,1:ncol(tendance)]

matriceCorrelation <- t(injectionOrthogonal) %*% matriceCorrelation %*% injectionOrthogonal
y_connus <- c(t(injectionOrthogonal) %*% y_connus)

	Prior <- evaluePriorREL1( x_connus, regularite, longueur_correlation, indice_derive, tendance)
	
	Posterior <- Prior * det(matriceCorrelation)^(-1/2) * abs(det(t(injectionParallele) %*% tendance)^(-1)) * (t(y_connus) %*% solve(matriceCorrelation, y_connus) )^(-(nrow(x_connus) - ncol(tendance))/2)

#print(det(t(injectionParallele) %*% tendance)^(-1))

	Posterior
}

setwd("../Rastrigin_dim10_Univ_LHSoptimise_vraiGibbs_jouet")
x_connus <- as.matrix(read.table("planXP.txt"))
y_connus <- scan("observations.txt")
tendance <- as.matrix(read.table("tendance.txt"))

base_longueur_correlation <- c(1,0.01,rep(1,8))

post_dim1 <- NULL
abs_dim1 <- NULL

abs_dim1 = c(abs_dim1,0.01*1:10)
for(i in 1:10)
{
	nouveau_longueur_correlation <- base_longueur_correlation
	nouveau_longueur_correlation[2] <- 0.01*i
	post_dim1 = c(post_dim1, evaluePosteriorREL1(x_connus,y_connus, 2.5, nouveau_longueur_correlation,2,tendance) )
}

abs_dim1 = c(abs_dim1,0.002*1:10)
for(i in 1:10)
{
	nouveau_longueur_correlation <- base_longueur_correlation
	nouveau_longueur_correlation[2] <- 0.002*i
	post_dim1 = c(post_dim1, evaluePosteriorREL1(x_connus,y_connus, 2.5, nouveau_longueur_correlation,2,tendance) )
}

abs_dim1 = c(abs_dim1,0.1*1:10)
for(i in 1:10)
{
	nouveau_longueur_correlation <- base_longueur_correlation
	nouveau_longueur_correlation[2] <- 0.1*i
	post_dim1 = c(post_dim1, evaluePosteriorREL1(x_connus,y_connus, 2.5, nouveau_longueur_correlation,2,tendance) )
}

abs_dim1 = c(abs_dim1,(0.1*1:10)+1)
for(i in 1:10)
{
	nouveau_longueur_correlation <- base_longueur_correlation
	nouveau_longueur_correlation[2] <- 1+0.1*i
	post_dim1 = c(post_dim1, evaluePosteriorREL1(x_connus,y_connus, 2.5, nouveau_longueur_correlation,2,tendance) )
}

plot(abs_dim1,post_dim1)
x11()

base_longueur_correlation <- c(rep(1,5),1,rep(1,4))
...
{
	nouveau_longueur_correlation <- base_longueur_correlation
	nouveau_longueur_correlation[6] <- 0.01*i
	post_dim1 = c(post_dim1, evaluePosteriorREL1(x_connus,y_connus, 2.5, nouveau_longueur_correlation,6,tendance) )
}
...
{
	nouveau_longueur_correlation <- base_longueur_correlation
	nouveau_longueur_correlation[6] <- 0.1*i
	post_dim1 = c(post_dim1, evaluePosteriorREL1(x_connus,y_connus, 2.5, nouveau_longueur_correlation,6,tendance) )
}

abs_dim1 = c(abs_dim1,0.001*1:10)
for(i in 1:10)
{
	nouveau_longueur_correlation <- base_longueur_correlation
	nouveau_longueur_correlation[6] <- 0.001*i
	post_dim1 = c(post_dim1, evaluePosteriorREL1(x_connus,y_connus, 2.5, nouveau_longueur_correlation,6,tendance) )
}
...
{
	nouveau_longueur_correlation <- base_longueur_correlation
	nouveau_longueur_correlation[6] <- 1+0.1*i
	post_dim1 = c(post_dim1, evaluePosteriorREL1(x_connus,y_connus, 2.5, nouveau_longueur_correlation,6,tendance) )
}
...
for(i in 1:10)
{
	nouveau_longueur_correlation <- base_longueur_correlation
	nouveau_longueur_correlation[6] <- 0.01*i
	pre_dim1 = c(pre_dim1, evaluePriorREL1(x_connus, 2.5, nouveau_longueur_correlation,6,tendance) )
}
...
{
	nouveau_longueur_correlation <- base_longueur_correlation
	nouveau_longueur_correlation <- 0.01*i
	pre_dim1 = c(pre_dim1, evaluePriorIsotropeREL1(x_connus, 2.5, nouveau_longueur_correlation,tendance) )
}
...
{
	nouveau_longueur_correlation <- base_longueur_correlation
	nouveau_longueur_correlation <- 0.1*i
	pre_dim1 = c(pre_dim1, evaluePriorIsotropeREL1(x_connus,2.5, nouveau_longueur_correlation,tendance) )
}
...
{
	nouveau_longueur_correlation <- base_longueur_correlation
	nouveau_longueur_correlation <- 0.001*i
	pre_dim1 = c(pre_dim1, evaluePriorIsotropeREL1(x_connus,2.5, nouveau_longueur_correlation,tendance) )
}

base_longueur_correlation <- c(0.01,rep(1,4),1,rep(1,4))
...
abs_dim1 = c(abs_dim1,1:10)
for(i in 1:10)
{
	nouveau_longueur_correlation <- base_longueur_correlation
	nouveau_longueur_correlation[6] <- i
	post_dim1 = c(post_dim1, evaluePosteriorREL1(x_connus,y_connus, 2.5, nouveau_longueur_correlation,6,tendance) )
}

evaluePriorREL2 <- function( x_connus, regularite, longueur_correlation, indice_derive, tendance)
    {
    Noyau <- creeMaternTensorise ( variance= 1, longueur= longueur_correlation, regularite= regularite)
...
evaluePosteriorREL2 <- function( x_connus, y_connus, regularite, longueur_correlation, indice_derive, tendance)
{
    Noyau <- creeMaternTensorise ( variance= 1, longueur= longueur_correlation, regularite= regularite)
	...
	Prior <- evaluePriorREL2( x_connus, regularite, longueur_correlation, indice_derive, tendance)
...
evalueVraisemblanceIntegreeKU <- function(x_connus, y_connus, regularite, longueur_correlation, tendance)
    {
    Noyau <- creeMaternTensorise ( variance= 1, longueur= longueur_correlation, regularite= regularite)
	...
	ProjectionBerger <- diag(rep(1,nrow(matriceCorrelation))) - tendance %*% solve(t(tendance) %*% solve(matriceCorrelation,tendance),t(tendance) %*% solve(matriceCorrelation))
print((c(t(ProjectionBerger %*%   y_connus) %*% solve(matriceCorrelation,ProjectionBerger %*% y_connus)))^(-(nrow(matriceCorrelation)-ncol(tendance))/2))
    Vraisemblance_integree <- det(matriceCorrelation)^(-1/2)* det(t(tendance) %*% solve(matriceCorrelation,tendance))^(-1/2) * (c(t(ProjectionBerger %*%   y_connus) %*% solve(matriceCorrelation,ProjectionBerger %*% y_connus)))^(-(nrow(x_connus)-ncol(tendance))/2)
print(Vraisemblance_integree)
    Vraisemblance_integree
}

evalueVraisemblanceIntegreeKUW <- function(x_connus, y_connus, regularite, longueur_correlation, tendance)
    {
    Noyau <- creeMaternTensorise ( variance= 1, longueur= longueur_correlation, regularite= regularite)
...
matriceCorrelation <- t(injectionOrthogonal) %*% matriceCorrelation %*% injectionOrthogonal
    ...
    Vraisemblance_integree <- det(matriceCorrelation)^(-1/2)* (sum(y_connus* solve(matriceCorrelation,y_connus)))^(-nrow(matriceCorrelation)/2)
    Vraisemblance_integree
}
...
VraisemblanceIntegree2DKU<- function(t,autre_longueur_correlation,x_connus,y_connus,regularite=1.5,tendance)
    {
    taille <- length(t)
    res <- rep(NA,taille)
    ...
    for (i in 1:taille)
        {
        res[i] <- evalueVraisemblanceIntegreeKU(x_connus = x_connus, y_connus = y_connus, regularite = regularite, longueur_correlation = c(t[i],autre_longueur_correlation),tendance=tendance)
        }
...
VraisemblanceIntegree2DKUW<- function(t,autre_longueur_correlation,x_connus,y_connus,regularite=1.5,tendance)
    {
    taille <- length(t)
    res <- rep(NA,taille)
    ...
    for (i in 1:taille)
        {
        res[i] <- evalueVraisemblanceIntegreeKUW(x_connus = x_connus, y_connus = y_connus, regularite = regularite, longueur_correlation = c(t[i],autre_longueur_correlation),tendance=tendance)
        }
...
abscisses <- c(0.01*1:100)
ordonnees<-VraisemblanceIntegree2D(abscisses,autre_longueur_correlation=0.1,x_connus=x_connus20, y_connus = y_connus20)
